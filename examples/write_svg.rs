//! Write a simple CAD drawing to SVG.
//!
//! Creates a small drawing with various entity types and exports it to
//! `drawing.svg` in the current directory.
//!
//! Run with:
//! ```sh
...
use acadrust::{
    CadDocument, EntityType, SvgWriter,
...
fn main() -> acadrust::error::Result<()> {
    ...
    // Multi-line text
    let mut mtext = MText::with_value("Hello\\PSVG!", Vector3::new(60.0, 5.0, 0.0));
    mtext.height = 5.0;
    doc.add_entity(EntityType::MText(mtext))?;

    // Export to SVG
    let path = "drawing.svg";
    SvgWriter::new(&doc).write_to_file(path)?;
    println!("Wrote {path}");
